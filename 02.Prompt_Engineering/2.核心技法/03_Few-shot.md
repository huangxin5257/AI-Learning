...
**质量比数量更重要**：1 个高质量示例通常优于 5 个平庸示例；示例数量超过 4～8 个后边际收益趋近于零。

### ② 底层原理

研究表明（Brown et al. 2020 GPT-3），增加示例数量的收益曲线呈对数增长：
- 0→1 个示例：收益最大（激活 in-context learning 模式）
...
关键发现：**示例质量比数量更重要**。质量定义：代表性强、格式一致、推理链正确、覆盖边界情况。
